component  hint="This is persistence implementation to persist/retrieve category information from database" output="true"   {
	...
	function saveCategory(categoryName,categoryDescription) returntype="cfsummit.product.model.category"   access="public"     {
		...
		queryexecute("insert into category_table (category_name,category_description) values(?,?)",
			...
			category = createObject("component", "cfsummit.product.model.category" );
	...
	function getCategoryDetailsById(categoryIdArg) returntype="cfsummit.product.model.brand" access="public" {
	...
	function getCategories() returntype="cfsummit.product.model.category[]" access="public" {
