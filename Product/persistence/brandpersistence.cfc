...
{
	function saveBrand(brandName,brandDescription) returntype="CFSummit.Product.model.brand" access="public"     {
		
		queryexecute("insert into brand_table (brand_name,brand_description) values(?,?)",
			[brandName,brandDescription],{result="result"});
			
			brand = createObject("component", "CFSummit.Product.model.brand");
	...
	function getBrands() returntype="CFSummit.Product.model.brand[]" access="public"      {
	...
	function getBrandDetailsById(brandIdArg) returntype="CFSummit.Product.model.brand" access="public" {
